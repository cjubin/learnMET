...
We use here the package **stacks** (https://stacks.tidymodels.org/index.html), incorporated in the tidymodels
...
```
The object of class `r class(rescv0_1 )` contains 3 elements, named `r cat(names(rescv0_1 )[1])`, `r cat(names(rescv0_1 )[2])` and `r cat(names(rescv0_1 )[3])`.\

### list_results_cv

The first element `r cat(names(rescv0_1 )[1])` is a list of `r cat(class(rescv0_1 $list_results_cv[[1]]))` objects, and has a length equal to the number of training/test sets partitions, which is determined by the cross-validation scenario.\
...
```
Within each of this `r cat(class(rescv0_1 $list_results_cv[[1]]))` object, 9 elements are provided:
...
```
To get the root mean square error between predicted and observed values for each environment, we extract the **cor_pred_obs**.
```{r}
