...
```
The object of class `class(rescv0_1)` contains 3 elements, named `cat(names(rescv0_1)[1])`, `cat(names(rescv0_1)[2])` and `cat(names(rescv0_1)[3])`.\

### list_results_cv

The first element `cat(names(rescv0_1)[1])` is a list of `cat(class(rescv0_1$list_results_cv[[1]]))` objects, and has a length equal to the number of training/test sets partitions, which is determined by the cross-validation scenario.\
...
```
Within each of this `cat(class(rescv0_1$list_results_cv[[1]]))` object, 9 elements are provided:
