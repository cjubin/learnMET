...
ALE_plot_split <-
  ...
  function(object,
    ...
    # or based on a minimum number of different feature values that should
    # be contained within each interval.
    if (!is.null(nb_bins)) {
      ...
      nb_bins <-
        round(length(unique(x_train[, variable])) / min_interval_points)
      s <-
        split(x_train, Hmisc::cut2(x_train[, variable], g = nb_bins))
