...
#' a `DL_reg_3` (neural network) regression model.
#'
#' @description
...
#'
#' @return A `list` object of class `DL_reg_3` with the following items:
#'   \describe{
...
#' @name DL_reg_3
#' @export
new_DL_reg_3 <- function(split = NULL,
                         trait = NULL,
                         geno = NULL,
                         env_predictors = NULL,
  ...
  if (type_location_info == "lon_lat_numeric" &
    ...
    rec <- recipe(~ . ,
      ...
      step_normalize(
        recipes::all_numeric(),-recipes::all_outcomes(),-tidyselect::starts_with('PC')
      )
...
#' @rdname DL_reg_3
#' @aliases new_DL_reg_3
#' @export
DL_reg_3 <- function(split,
                     trait,
                     geno,
                     env_predictors,
                     info_environments,
