...
apply_pcs_G_Add <- function(split, geno, num_pcs, ...) {
  print(num_pcs)
  ...
  snps_data_tr <- as.data.frame(bake(rec_snps,new_data = geno_training))
  row.names(snps_data_tr) <- snps_data_tr$geno_ID
  snps_data_te <- as.data.frame(bake(rec_snps,new_data = geno_test))
