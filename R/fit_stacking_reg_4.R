#' @rdname fit_cv_split
#' @export
fit_cv_split.stacking_reg_4 <- function (object,
                                         seed,
                                         inner_cv_reps = 1,
                                         inner_cv_folds = 5,
                                         kernel_G = 'rbf',
  ...
  training = object[['training_A']]
  test = object[['test_A']]
  
  rec_G_add = object[['rec_G_add']]
  rec_G_dom = object[['rec_G_dom']]
  rec_E = object[['rec_E']]
  rec_all = object[['rec_all']]
  
  trait = as.character(rec_G_add$var_info[rec_G_add$var_info$role == 'outcome', 'variable'])
  ...
  svm_wflow_G_dom <-
    ...
    workflows::add_recipe(rec_G_dom)
  ...
  svm_wflow_G_add <-
    workflows::workflow() %>%
    workflows::add_model(svm_spec_G) %>%
    workflows::add_recipe(rec_G_add)
  ...
  cat('Support vector regression with genomic kernel (dominance effects) done!')
  ...
  cat('Support vector regression with genomic kernel (additive effects) done!')
  ...
  METData_data_st <-
    stacks::stacks() %>%
    stacks::add_candidates(svm_res_G_add) %>%
    stacks::add_candidates(svm_res_G_dom) %>%
  ...
  parameters_collection_G_add <-
    METData_model_st %>% stacks::collect_parameters('svm_res_G_add')
  parameters_collection_G_dom <-
    METData_model_st %>% stacks::collect_parameters('svm_res_G_dom')
  ...
  res_fitted_split <- structure(
    list(
      ...
      'parameters_collection_G_add' = as.data.frame(parameters_collection_G_add),
      'parameters_collection_G_dom' = as.data.frame(parameters_collection_G_dom),
