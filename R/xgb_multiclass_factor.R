...
#' @title 
#' blabla
#' @name xgb_multiclass_factor
#' @export
new_xgb_multiclass_factor <- function(split,
                                      trait,
                                      geno_data,
                                      env_predictors,
                                      info_environments,
  ...
  if (class(split[['training']][, trait]) == c('integer')) {
    split[['training']][, trait] <-
      as.factor(split[['training']][, trait])
    split[['test']][, trait] <-
      as.factor(split[['test']][, trait])
    
  } else if (class(split[['training']][, trait]) == 'numeric' &
             length(unique(split[['training']][, trait])) > 20) {
    stop(paste(
      'The trait seems to be continuous (at least present more than 20 unique', 
      'values) and should therefore not be used with this method.'
    ))
  }
...
#' @rdname xgb_multiclass_factor
#' @aliases new_xgb_multiclass_factor
#' @export
xgb_multiclass_factor <- function(split,
                        trait,
                        geno_data,
                        env_predictors,
                        info_environments,
                        ...
                        lat_lon_included,
                        year_included) {
  validate_xgb_multiclass_factor(
    new_xgb_multiclass_factor(
      split = split,
      trait = trait,
      geno_data = geno_data,
...
validate_xgb_multiclass_factor <- function(x) {
  ...
  checkmate::assert_names(names(x),
                          must.include = c('training', 'test', 'rec', 'nb_classes'))
  
  checkmate::assert_class(x[['training']][, trait], 'factor')
  
  checkmate::assert_class(x[['test']][, trait], 'factor')
