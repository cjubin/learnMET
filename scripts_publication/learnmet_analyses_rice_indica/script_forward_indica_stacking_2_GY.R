...
rescv0_2 <- predict_trait_MET(
  ...
  seed = 100,
  save_processing = T,
  path_folder = '~/results_benchmarking_rice_indicaGY/stacking/fw2'
)
end_time <- Sys.time()
...
pred_df <- plyr::join(pred_df %>% dplyr::select(-GY,-year,-location),  as.data.frame(pheno_indica[pheno_indica$year%in%2012,]),by=c('IDenv','geno_ID') )
