...
params:
  ...
  path: 
    value: '/home/uni08/jubin1/Data/PackageMLpredictions/learnmet_plus/bglr_rice_data_indica/PHR'
...
```{r, include=FALSE,echo=FALSE} 
setwd("/home/uni08/jubin1/Data/PackageMLpredictions/learnmet_plus/bglr_rice_data_indica")
...
year_to_predict = as.numeric(params$env_to_predict)+2010
...
if (params$geno_info == 'PCs') {
  ...
  X_te=as.matrix(geno[which(geno$geno_ID%in%test$geno_ID&geno$geno_ID%notin%training$geno_ID),])
...
y_tr<-as.numeric(training$PHR)
y_te<-as.numeric(test$PHR)
y_tot <- as.data.frame(c(y_tr,y_te))
colnames(y_tot)<-'PHR'
y_tot$PHR[(length(y_tr)+1):nrow(y_tot)]<-NA

library(dplyr)
env_data <- load('climate_variables_indica.rda')  
env_data<- climate_variables_indica
...
fm=BGLR::BGLR(y=y_tot$PHR,ETA=ETA,nIter=nIter,burnIn=burnIn,saveAt=paste0(params$path,'/',params$env_to_predict,'fmobject_G_E_W_GW_'))
...
s <- y_pred %>%
  ...
  yardstick::metrics(PHR, y_pred) %>%
  dplyr::mutate(.estimate = format(round(.estimate, 2), big.mark = ","))

s2 <- y_pred %>%
  dplyr::bind_cols(test) %>%
  dplyr::group_by(IDenv) %>%
  dplyr::summarize(COR=cor(PHR,y_pred,method = 'pearson'))
