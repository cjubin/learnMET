...
rescv0_2 <- predict_trait_MET(
  ...
  seed = 100,
  save_processing = T,
  path_folder = '~/results_benchmarking_rice_japonica/GY/stacking/fw2'
)
end_time <- Sys.time()
...
pred_df <- plyr::join(pred_df %>% dplyr::select(-GY,-PH,-year,-location),  as.data.frame(pheno_japonica[pheno_japonica$year%in%2011,]),by=c('IDenv','geno_ID') )
