...
params:
  ...
  PLS_nb_comp: 
    value: 200
  phenos_file:
    value: 'pheno_indica.rda'
  geno_file: 'geno_indica.rda'
  env_to_predict:
    value: 3
  trait:
    value: 'yld_bu_ac'
  ...
  path: 
    value: '/home/uni08/jubin1/Data/PackageMLpredictions/learnmet_plus/bglr_g2f_fw/fw1'
...
env_to_predict=as.character(unique(phenos$IDenv)[params$env_to_predict])

training = phenos[which(phenos$year%in%2014),]
test = phenos[which(phenos$year%in%c(2015)),]
...
y_tr<-as.numeric(training$yld_bu_ac)
y_te<-as.numeric(test$yld_bu_ac)
y_tot <- as.data.frame(c(y_tr,y_te))
colnames(y_tot)<-'yld_bu_ac'
y_tot$yld_bu_ac[(length(y_tr)+1):nrow(y_tot)]<-NA
library(dplyr)
env_data_training <- readRDS('/home/uni08/jubin1/Data/PackageMLpredictions/learnmet_plus/benchmarking_g2f/METData_training_fw_1.RDS')$env_data
env_data_test <- readRDS('/home/uni08/jubin1/Data/PackageMLpredictions/learnmet_plus/benchmarking_g2f/METData_test_fw_1.RDS')$env_data
row.names(env_data_training) <- env_data_training$IDenv
row.names(env_data_test) <- env_data_test$IDenv

env_data_training <- env_data_training %>% dplyr::select(-IDenv,-location,-year)

env_data_test <- env_data_test %>% dplyr::select(-IDenv,-location,-year)

env_variables <- which(apply(env_data_training, 2, var) != 0)
env_data_training <- env_data_training[,env_variables]

env_data_test <- env_data_test[,env_variables]
...
W_tot<-as.data.frame(rbind(training%>%select(all_of(colnames(env_data_training))),test%>%select(all_of(colnames(env_data_test)))))
W_tot <- W_tot %>% dplyr::select(-IDenv)
W_tot <- as.matrix(W_tot/sqrt(ncol(W_tot)))
saveRDS(W_tot,file=paste0(params$path,'/','W_tot',params$env_to_predict,'.RDS'))
WW<-tcrossprod(W_tot)
K<-ZAZ*WW
diag(K)<-diag(K)+1/200
K<-K/mean(diag(K))
...
fm=BGLR::BGLR(y=y_tot$yld_bu_ac,ETA=ETA,nIter=nIter,burnIn=burnIn,saveAt=paste0(params$path,'/','fmobject_G_E_W_GW'))
...
s <- y_pred %>%
  ...
  yardstick::metrics(yld_bu_ac, y_pred) %>%
  dplyr::mutate(.estimate = format(round(.estimate, 2), big.mark = ","))

s2 <- y_pred %>%
  dplyr::bind_cols(test) %>%
  dplyr::group_by(IDenv) %>%
  dplyr::summarize(COR=cor(yld_bu_ac,y_pred,method = 'pearson'))
